...
//    }
    ...
    // Edited version from LivingEntityRenderer
    public static RenderLayer getRenderLayer(Identifier texture, boolean visible, boolean revealed, boolean outlined, boolean emissive) {
        /*if (revealed) {
            return RenderLayer.getItemEntityTranslucentCull(texture);
        } else*/ if (visible || revealed) {
            return emissive?RenderLayer.getEntityTranslucentEmissive(texture):RenderLayer.getEntityTranslucent(texture);
        } else {
            return outlined ? RenderLayer.getOutline(texture) : null;
        }
    }

    public static int getRenderColor(AbstractClientPlayerEntity entity, boolean revealed) {
        int argb = ColorHelper.Argb.fromFloats (revealed?0.15f:1,1, 1, 1);
